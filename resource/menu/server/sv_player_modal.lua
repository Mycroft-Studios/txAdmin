...
RegisterNetEvent('txsv:req:getAdaptiveCard', function(resource)
  local src = source
  ...
  local allow = PlayerHasTxPermission(src, 'players.teleport')
  if allow then
    local resourceData = TX_UIADDONS[resource]
    local exportName = nil
    ...
    if not exportName then return end
    print(src)
    local response = exports[resource][exportName](nil, src)
    TriggerClientEvent("txcl:RecieveAdaptiveCard", src, response)
  end
end)

RegisterNetEvent('txsv:req:ACActionSubmit', function(data)
  local src = source
  ...
  if allow then
    ...
    for k,v in pairs(resourceData) do
      if v.type == "player" then
        exportName = v.onSubmit
        break
      end
    end
    if not exportName then return end
    exports[resource][exportName](nil, src, inputs)
  end
end)